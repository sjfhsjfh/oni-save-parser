...
function taggedParser(tag, arg1, arg2) {
    ...
    return function* (...args) {
        ...
        const resolvedInstance = instanceName && typeof instanceName === "function"
            ? instanceName(...args)
            : instanceName;
        yield instructions_1.TaggedParseInstruction.start(resolvedTag, resolvedInstance);
        const result = yield* parser(...args);
        yield instructions_1.TaggedParseInstruction.end(resolvedTag, resolvedInstance);
        return result;
    };
}
