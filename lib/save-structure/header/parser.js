...
function* parseHeader() {
    const buildVersion = yield new parser_1.ReadUInt32Instruction();
    const headerSize = yield new parser_1.ReadUInt32Instruction();
    const headerVersion = yield new parser_1.ReadUInt32Instruction();
    const isCompressed = headerVersion >= 1 ? Boolean(yield new parser_1.ReadUInt32Instruction()) : false;
    const infoBytes = yield new parser_1.ReadBytesInstruction(headerSize);
